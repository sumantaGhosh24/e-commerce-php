...
<div class="min-h-screen pt-8 bg-white container mx-auto">
    <div class="overflow-x-scroll">
        <div class="flex items-center justify-between">
            <h2 class="text-2xl font-bold text-center">Manage Sizes</h2>
        ...
        <table class="min-w-full bg-white rounded-lg shadow-md mx-auto mt-5">
            <thead>
                <tr class="bg-gray-200 text-gray-600 uppercase text-sm leading-normal">
                    ...
                    <th class="py-3 px-6 text-left">Order By</th>
...
<script>
    $(document).ready(function () {
        function fetchSizes() {
            $.ajax({
                url: "http://localhost:3000/admin/includes/size.php?admin=true",
                type: "get",
                success: function (result) {
                    ...
                    sizes.forEach(size => {
                        $("#sizes").append(`
                            <tr>
                                <td class="py-3 px-6 text-left">${size.id}</td>
                                <td class="py-3 px-6 text-left">${size.size}</td>
                                <td class="py-3 px-6 text-left">${size.order_by}</td>
                                <td class="py-3 px-6 text-left">${size.status == "1" ? "Active" : "Deactive"}</td>
                                <td class="py-3 px-6 text-left">${size.createdAt}</td>
                                <td class="py-3 px-6 text-left">${size.updatedAt}</td>
                                <td class="py-3 px-6 text-left flex items-center gap-3">
                                    <a href="./update-size.php?id=${size.id}" class="w-fit bg-green-500 text-white px-4 py-2 rounded-md hover:bg-green-600 transition-colors disabled:bg-green-200">Update</a>
                                    <form class="size_delete_form">
        ...
        $(document).on("submit", ".size_delete_form", function (e) {
            e.preventDefault();
